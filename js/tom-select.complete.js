/**
 * Tom Select v2.0.0
 * Bug fixed by https://github.com/githabas
 *
 *
...
(function (global, factory) {
	...
	class MicroEvent {
		...
		trigger(events, ...args) {
			const self = this;
			forEvents(events, event => {
				if (!(event in self._events)) return;
	...
	const generateDiacritics = () => {
		...
		code_points.forEach(code_range => {
			for (let i = code_range[0]; i <= code_range[1]; i++) {
				...
				const patt = new RegExp(arrayToPattern(diacritics[latin]), 'iu');
		...
		latin_chars.sort((a, b) => a.length - b.length).forEach(latin => {
			const substrings = allSubstrings(latin);
			const pattern = substrings.map(sub_pat => {
	...
	class Sifter {
		...
		_getScoreFunction(search) {
			...
			const scoreObject = function () {
				...
				return function (token, data) {
					let sum = 0; // is the token specific to a field?
	...
	const debounce_events = (self, types, fn) => {
		...
		const event_args = {}; // override trigger method
	...
	function getSettings(input, settings_user) {
		const settings = Object.assign({}, defaults, settings_user);
	...
	class TomSelect extends MicroPlugin(MicroEvent) {
		 ...
		 */
		canSelect(option) {
			return !!(this.isOpen && option && this.dropdown_content.contains(option));
		}
		...
		canLoad(value) {
			...
			return !this.loadedSearches.hasOwnProperty(value);
		}
		...
		refreshOptions(triggerDropdown = true) {
			let i, j, k, n, optgroup, optgroups, html, has_create_option, active_value, active_group;
			let create;
			const groups = {};
			const groups_order = [];
			const self = this;
		...
		addOption(data, user_created = false) {
			...
			if (!key || self.options.hasOwnProperty(key)) {
				return false;
			}
		...
		addOptionGroup(id, data) {
			...
			const hashed_id = this.registerOptionGroup(data)
			if (hashed_id) {
		...
		removeOption(value, silent) {
			...
			self.options[value]['$option'].removeAttribute("selected")
			self.lastQuery = null;
		...
		removeItem(item = null, silent) {
			...
			//-- klaida, nes nebūtinai pirmą nori trint
			// self.items.splice(i, 1);
			self.items = self.items.filter(e => e !== value)

			self.lastQuery = null;
			// if (!self.settings.persist && self.userOptions.hasOwnProperty(value)) {
			// if (self.options.hasOwnProperty(value) || self.userOptions.hasOwnProperty(value)) {
	...
	function checkbox_options () {
		...
		const UpdateCheckbox = function UpdateCheckbox(option) {
			setTimeout(() => {
		...
		self.on('item_remove', value => {
			...
			if (option) {
				...
				// selected class won't be removed yet
	...
	function remove_button (userOptions) {
		...
		self.hook('after', 'setupTemplates', () => {
			...
			self.settings.render.item = (data, escape) => {
				const rendered = getDom(orig_render_item.call(self, data, escape));
				...
				addEvent(close_button, 'click', evt => {
					...
					const value = rendered.dataset.value;
	...
	function virtual_scroll () {
		...
		function canLoadMore(query) {
			...
			return query in pagination && pagination[query];
		}
...
const tomSelect = function (el, opts) {
